...
int fun_incre(int *fila);  //Alterado: uso de pré-incremento e pré-decremento (Alteração 1)
void flush_in ();
float calcValor (float preco, float qtd);
int fun_invalidos (const float prEta, const float prGas, const float prAdit, const float tFila); //Alterado: adicionado const nos parâmetros (Alteração 2)
int fun_escreva();
char Fun_Tcar();
...
int main (void) {
  ...
  //VARIÁVEIS
    ...
    struct Tcarro carros [(int)tFila]; //Inicializa o array com tamanho definido por tFila (float convertido para int) para evitar tamanho indefinido.
...
//Alteração 1: uso de pré-incremento e pré-decremento
